    ...
    for (idx3[0] = 0; idx3[0] < d3.dim[0];idx3[0]++)
        for (idx4[2] = 0; idx4[2] < d4.dim[2];idx4[2]++)
            for (idx4[3] = 0; idx4[3] < d4.dim[3];idx4[3]++) {
                ...
                ASSERT_FLOAT_EQ(Msum.elements[0], M.elements[lin]) << " lin: " << lin << " Msumelem: " << Msum.elements[lin] << " Melem: " << M.elements[lin];
            }
}

TEST(comparaison, values){
  int a=1,b=2;
  EXPECT_LT(b,a);
  /*EXPECT_LE(b,a);
  EXPECT_GT(a,b);
